...
// THIRD-PARTY.md for attribution details. SRO's own key derivation is not upstream's
// and is ours.
use std::fmt;
...
impl Blowfish {
    ...
    pub fn encrypt(&self, data: &mut [u8]) {
        data.chunks_exact_mut(BLOCK_SIZE)
            .for_each(|data| self.encrypt_block(data.try_into().expect("encrypt to block itself")))
    }

    pub fn decrypt(&self, data: &mut [u8]) {
        data.chunks_exact_mut(BLOCK_SIZE)
            .for_each(|data| self.decrypt_block(data.try_into().expect("encrypt to block itself")))
    }
