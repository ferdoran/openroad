...
#[darling(attributes(sro_packet))]
pub(crate) struct PacketFieldArgs {
...
#[proc_macro_error]
#[proc_macro_derive(Serialize, attributes(sro_packet))]
pub fn derive_serialize(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input);
    let args = PacketArgs::from_derive_input(&input).expect("i failed");
    ...
    let output = quote! {
        impl Serialize for #ident {
            fn serialize_to(&self, mut buf: &mut bytes::BytesMut) {
                #output
            }
        }
...
#[proc_macro_error]
#[proc_macro_derive(ByteSize, attributes(sro_packet))]
    ...
    match ty {
        ...
        Type::Path(path) => {
                ...
                || full_name == "std::string::String"
            {
                ...
                match path.path.segments.last().expect("i failed").arguments {
                    PathArguments::None => {
...
fn get_variant_value<T: Spanned + ToTokens>(source: &T, value: usize, size: usize) -> Expr {
    ...
    syn::parse_str(&format!("{}{}", value, ty)).expect("i failed")
}
