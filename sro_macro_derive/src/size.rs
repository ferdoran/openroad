...
use syn::{spanned::Spanned, Data, Field, Fields, Index, Type};

use crate::{get_type_of, PacketArgs, PacketFieldArgs, UsedType};

pub(crate) fn size(ident: &Ident, data: &Data, args: PacketArgs) -> TokenStream {
    match *data {
...
fn generate_size_for(field: &Field, ident: TokenStream) -> TokenStream {
    ...
    let field_args = PacketFieldArgs::from_attributes(&field.attrs).expect("i failed");
    match ty {
        UsedType::Primitive => {
            quote_spanned!(field.span() => #ident.byte_size())
        }
        UsedType::String => {
            // Mirror the serialize derive: UTF-16 strings occupy 2 bytes per
            // code unit, so byte-length math must use `encode_utf16().count()`.
            match field_args.size.unwrap_or(1) {
                2 => quote_spanned! { field.span() =>
                    2 + #ident.encode_utf16().count() * 2
                },
                size => quote_spanned! { field.span() =>
                    2 + #ident.len() * #size
                },
            }
        }
        ...
        UsedType::Collection(inner) => {
            ...
            if length_type == "break" || length_type == "has-more" || length_type == "by-size-field"
            {
        ...
        UsedType::Option(inner) => {
            let inner_ty = get_type_of(inner);
            let inner_ts = generate_size_for_inner(inner, &inner_ty, quote!(elem));
            // `when`-conditional options have no presence flag (see the
            // serialize derive), so their size is exactly the inner value's
            // size when present. Plain options add the flag byte.
            if field_args.when.is_some() {
                quote_spanned! { field.span() =>
                    #ident.as_ref().map(|elem| #inner_ts).unwrap_or(0)
                }
            } else {
                let size = 1usize;
                quote_spanned! { field.span() =>
