// Blowfish, derived from the RustCrypto `block-ciphers` implementation
// (https://github.com/RustCrypto/block-ciphers), copied out because their
// exposed API is rather unwieldy for SRO's use of the primitive.
//
// Upstream is dual-licensed Apache-2.0 OR MIT, Copyright (c) RustCrypto
// Developers. Both licences require this notice to travel with the code; see
// THIRD-PARTY.md for attribution details. The PK2 salt fold
// (`gen_final_blowfish_key_inplace`) is SRO's, not upstream's.
...
impl Blowfish {
    /// Derive the cipher from a user-supplied [`Pk2Key`].
    ///
    /// There is intentionally no `Default`: without a key there is no cipher,
    /// and a caller that has not been given one must surface that as an error
    /// rather than fall back to something built in.
    pub fn from_key(key: &Pk2Key) -> Result<Self, InvalidKey> {
        Blowfish::new(key.key(), key.salt())
    }
}
